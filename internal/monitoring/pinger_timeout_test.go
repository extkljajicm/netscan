...
func TestTimeoutParameterPropagation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			StartPinger(ctx, nil, dev, tt.interval, tt.timeout, writer, stateMgr, limiter, &counter, 10, 5*time.Minute)
...
func TestTimeoutNotHardcoded(t *testing.T) {
	...
	for _, timeout := range timeouts {
		...
		StartPinger(ctx, nil, dev, 100*time.Millisecond, timeout, writer, stateMgr, limiter, &counter, 10, 5*time.Minute)
